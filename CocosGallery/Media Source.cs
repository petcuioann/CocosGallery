...
namespace CocosGallery {
    public class IncrementalMediaSource : ObservableCollection<MediaItem>, ISupportIncrementalLoading {
        ...
        /// <summary>Incrementally loads more media items into the collection.</summary>
        /// <remarks>Complexity: O(n)</remarks>
        public Windows.Foundation.IAsyncOperation<LoadMoreItemsResult> LoadMoreItemsAsync(uint count) => AsyncInfo.Run(async (cancellationToken) => {
            if (_queryResult == null) {
                ...
                try { if (!string.IsNullOrEmpty(_folderPath)) folder = await StorageFolder.GetFolderFromPathAsync(_folderPath); } catch { }
