    ...
    public sealed partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            this.RootGrid.Loaded += (s, e) => {
                ...
                if (!ViewModel.IsAutoShrinkSidebar)
                {
        ...
        // pre: window was hidden and is being prepared to be shown again
        // post: resets selection, viewer state, and navigation back to root
        // except: none
        // complexity: O(1) mostly
        public void ResetToStartingPoint()
        {
            this.ViewModel.ClearSelection();
            if (MainViewModel.IsViewerOpen)
            {
            ...
            if (this.ViewModel.SelectedFolder != this.ViewModel.Folders.FirstOrDefault())
            {
                this.ViewModel.SelectedFolder = this.ViewModel.Folders.FirstOrDefault();
            }
        ...
        private async void CrossMemoryToggle_Click(object sender, RoutedEventArgs e)
        {
            if (sender is ToggleMenuFlyoutItem toggle)
            {
                var dialog = new ContentDialog
                {
                    ...
                    Content = "The application needs to restart for the cross-memory selection setting to take effect. Would you like to restart now?",
        ...
        // pre: memory and media items are actively held by viewers
        // post: viewer sources and selection are cleared to allow garbage collection
        // except: none
        // complexity: O(n) where n is the number of media items cleared
        public void ClearViewers()
        {
            this.ViewerImage.Source = null;
            this.ViewerVideo.Source = null;
            this.ViewModel.MediaItems.Clear();
            this.ViewModel.ClearSelection();
        }

        // pre: app window is attempting to close
        // post: hides the window if it is the last active window, otherwise allows closing
        // except: none
        // complexity: O(1)
        ...
        {
            this.AppWindow.Show();
            if (this.AppWindow.Presenter is OverlappedPresenter presenter && presenter.State == OverlappedPresenterState.Minimized) presenter.Restore();
        ...
        private void UpdateTitleBarRegions()
        {
            ...
            var dragRect = new RectInt32(0, 0, this.AppWindow.Size.Width, titleBarHeight);
            ...
            nonClientSource.SetRegionRects(NonClientRegionKind.Close, Array.Empty<RectInt32>());
            nonClientSource.SetRegionRects(NonClientRegionKind.Minimize, Array.Empty<RectInt32>());
            nonClientSource.SetRegionRects(NonClientRegionKind.Maximize, Array.Empty<RectInt32>());
        }

        private void TrafficLightClose_Click(object? sender, RoutedEventArgs? e)
        {
            if (App.ActiveWindows.Count == 1 && App.ActiveWindows.Contains(this))
            {
                this.AppWindow.Hide();
        ...
        {
            if (ViewModel.IsAutoShrinkSidebar && !ViewModel.IsSidebarVisible)
            {
                if (e.GetCurrentPoint(RootGrid).Position.X <= 20)
                {
                    ViewModel.IsSidebarVisible = true;
                    AnimateSidebar(true);
                }
        ...
        private void SettingsLight_Click(object sender, RoutedEventArgs e)
        {
            MenuFlyout? flyout = this.RootGrid.Resources["SettingsMenu"] as MenuFlyout;
            flyout?.ShowAt(SettingsAnchor, new FlyoutShowOptions { Placement = FlyoutPlacementMode.TopEdgeAlignedLeft });
        }

        private void AutoShrinkToggle_Changed()
        {
            if (ViewModel.IsAutoShrinkSidebar)
            {
                ViewModel.IsSidebarVisible = _isSidebarHovered;
                AnimateSidebar(_isSidebarHovered);
            }
            else
            {
                ViewModel.IsSidebarVisible = true;
        ...
        private void SidebarGrid_PointerExited(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (ViewModel.IsAutoShrinkSidebar && ViewModel.IsSidebarVisible)
            {
                var ptr = e.GetCurrentPoint(RootGrid).Position;
                if (ptr.X >= SidebarGrid.ActualWidth)
                {
        ...
        // pre: a valid layout state and viewmodel size is set
        // post: sidebar layout width and visibility are animated to reflect 'show' boolean
        // except: none
        // complexity: O(1) time
        private void AnimateSidebar(bool show)
        {
            ...
            double targetSidebarX = show ? 0 : -260;
            double targetPushWidth = (show && isPinned) ? 260 : 0;
            double targetBottomOffset = show ? 260 : 0;
            ...
            var sidebarAnim = new DoubleAnimation { To = targetSidebarX, Duration = duration, EasingFunction = ease };
            Storyboard.SetTarget(sidebarAnim, SidebarTranslate);
            Storyboard.SetTargetProperty(sidebarAnim, "X");

            var pushAnim = new DoubleAnimation { To = targetPushWidth, Duration = duration, EasingFunction = ease, EnableDependentAnimation = true };
            Storyboard.SetTarget(pushAnim, ContentPush);
            Storyboard.SetTargetProperty(pushAnim, "Width");

            var bottomAnim = new DoubleAnimation { To = targetBottomOffset, Duration = duration, EasingFunction = ease };
            Storyboard.SetTarget(bottomAnim, BottomControlsTranslate);
            Storyboard.SetTargetProperty(bottomAnim, "X");

            sb.Children.Add(sidebarAnim);
            sb.Children.Add(pushAnim);
            sb.Children.Add(bottomAnim);

            if (show) SidebarGrid.Visibility = Visibility.Visible;
            sb.Completed += (s, ev) => { if (!show) SidebarGrid.Visibility = Visibility.Collapsed; };
            sb.Begin();
        }
        ...
        private void OverflowMenu_Opened(object sender, object e)
        {
            CollapseToggleButton.Opacity = 0;
            CollapseToggleButton.IsHitTestVisible = false;
        }

        private void OverflowMenu_Closed(object sender, object e)
        {
            CollapseToggleButton.Opacity = 1;
            CollapseToggleButton.IsHitTestVisible = true;
        }

        // pre: visual tree is fully initialized and bounds are available
        // post: action panel is adjusted to horizontal, vertical or overflow state based on available width/height
        // except: none
        // complexity: O(1) time
        private void UpdateActionPanelLayout()
        {
            ...
            double h = this.RootGrid.ActualHeight;
            double w = this.RootGrid.ActualWidth;

            ActionPanelState targetState;

            if (ViewModel.IsPreferContextMenus) targetState = ActionPanelState.Overflow;
            else if (h > 650) targetState = ActionPanelState.Vertical;
            else if (w > 650) targetState = ActionPanelState.Horizontal;
            else targetState = ActionPanelState.Overflow;
        ...
        private void ApplyActionPanelState(ActionPanelState state)
        {
            ...
            BottomActionPanel.Children.Clear();
            CollapsibleButtons.Children.Clear();

            if (_actionSeparator != null)
            {
                if (state == ActionPanelState.Vertical)
                {
                    _actionSeparator.Width = 16; _actionSeparator.Height = 1; _actionSeparator.Margin = new Thickness(0, 4, 0, 4);
                }
                else
                {
                    _actionSeparator.Width = 1; _actionSeparator.Height = 24; _actionSeparator.Margin = new Thickness(4, 0, 4, 0);
                }
            }

            if (state == ActionPanelState.Overflow)
            {
                BottomActionPanel.Orientation = Orientation.Horizontal;
                BottomActionPanel.Children.Add(CollapseToggleButton);
                CollapseToggleButton.Flyout = null!;
                CollapsibleButtons.Visibility = Visibility.Collapsed;
            }
            else if (state == ActionPanelState.Vertical)
            {
                BottomActionPanel.Orientation = Orientation.Vertical;
                CollapsibleButtons.Orientation = Orientation.Vertical;
                CollapsibleButtons.Visibility = this.ViewModel.AreActionButtonsVisible ? Visibility.Visible : Visibility.Collapsed;
                CollapseToggleButton.Flyout = null!;

                var reversed = new List<UIElement>(_actionButtons);
                reversed.Reverse();
                foreach (var el in reversed) CollapsibleButtons.Children.Add(el);

                BottomActionPanel.Children.Add(CollapsibleButtons);
                BottomActionPanel.Children.Add(CollapseToggleButton);
            }
            else
            {
                BottomActionPanel.Orientation = Orientation.Horizontal;
                CollapsibleButtons.Orientation = Orientation.Horizontal;
                CollapsibleButtons.Visibility = this.ViewModel.AreActionButtonsVisible ? Visibility.Visible : Visibility.Collapsed;
                CollapseToggleButton.Flyout = null!;

                foreach (var el in _actionButtons) CollapsibleButtons.Children.Add(el);
            ...
            if (!_isHudVisible)
            {
                if (state == ActionPanelState.Horizontal) { BottomPanelTranslate.X = 0; BottomPanelTranslate.Y = 100; }
                else { BottomPanelTranslate.Y = 0; BottomPanelTranslate.X = -100; }
            }
            else
            {
                BottomPanelTranslate.X = 0; BottomPanelTranslate.Y = 0;
            }
        }

        private void UpdateToggleIcon()
        {
            bool isVis = this.ViewModel.AreActionButtonsVisible;
            if (_currentActionState == ActionPanelState.Overflow) ToggleIcon.Glyph = "\uE712";
            else if (_currentActionState == ActionPanelState.Vertical) ToggleIcon.Glyph = isVis ? "\uE70D" : "\uE70E";
            else ToggleIcon.Glyph = isVis ? "\uE76B" : "\uE76C";
        }

        private void CollapseToggleButton_Click(object sender, RoutedEventArgs e)
        {
            if (_currentActionState == ActionPanelState.Overflow)
            {
                MenuFlyout? flyout = this.RootGrid.Resources["OverflowMenu"] as MenuFlyout;
                flyout?.ShowAt(MenuAnchor, new FlyoutShowOptions { Placement = FlyoutPlacementMode.TopEdgeAlignedLeft });
                return;
            }

            this.ViewModel.ToggleActionButtons();
            CollapsibleButtons.Visibility = this.ViewModel.AreActionButtonsVisible ? Visibility.Visible : Visibility.Collapsed;
            UpdateToggleIcon();
        }

        private void ViewerOverlay_RightTapped(object sender, RightTappedRoutedEventArgs e)
        {
            MenuFlyout? flyout = this.RootGrid.Resources["OverflowMenu"] as MenuFlyout;
            flyout?.ShowAt(ViewerOverlay, e.GetPosition(ViewerOverlay));
        }
        ...
        // pre: show indicates the desired visibility state of the HUD
        // post: hud elements animate to requested state with proper debouncing
        // except: silences duplicate calls
        // complexity: O(1) time
        private void AnimateHud(bool show)
        {
            if (_isHudVisible == show) return;
            if (show && _isDebouncing) return;

            _isHudVisible = show;
            var duration = TimeSpan.FromSeconds(0.3);
            var ease = new ExponentialEase { EasingMode = EasingMode.EaseOut, Exponent = 4 };

            var topAnim = new DoubleAnimation { To = show ? 0 : -50, Duration = duration, EasingFunction = ease };
            Storyboard topSb = new Storyboard(); Storyboard.SetTarget(topAnim, TopPanelTranslate); Storyboard.SetTargetProperty(topAnim, "Y"); topSb.Children.Add(topAnim); topSb.Begin();

            var bottomAnim = new DoubleAnimation { Duration = duration, EasingFunction = ease };
            Storyboard bottomSb = new Storyboard(); Storyboard.SetTarget(bottomAnim, BottomPanelTranslate);

            if (_currentActionState == ActionPanelState.Horizontal)
            {
                bottomAnim.To = show ? 0 : 100;
                Storyboard.SetTargetProperty(bottomAnim, "Y");
                BottomPanelTranslate.X = 0;
            }
            else
            {
                bottomAnim.To = show ? 0 : -100;
            ...
            {
                StopIdleTimer();
                _lastHudHideTime = DateTime.Now;
                _isDebouncing = true;
                _ = Task.Run(async () => { await Task.Delay(500); _isDebouncing = false; });
            }
        }
        ...
        private void RootGrid_KeyDown(object sender, KeyRoutedEventArgs e)
        {
            if (this.ViewerOverlay.Visibility == Visibility.Visible)
            {
                ...
                if (!_isHudVisible) AnimateHud(true);
                StartIdleTimer();

                if (e.Key == VirtualKey.Left) _ = NavigateMedia(-1);
                else if (e.Key == VirtualKey.Right) _ = NavigateMedia(1);
                else if (e.Key == VirtualKey.Escape) CloseViewer_Click(null, null);
            }
            else
            {
                if (e.Key == VirtualKey.Escape && this.ViewModel.IsSelectMode)
                {
                    this.ViewModel.ClearSelection();
                    e.Handled = true;
                }
        ...
        private void ViewerOverlay_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            if (this.ViewerOverlay.Visibility == Visibility.Visible)
            {
        ...
        private void ViewerOverlay_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (currentTicks - _lastClickTicks < TimeSpan.TicksPerMillisecond * 500)
            {
                var ptOverlay = e.GetCurrentPoint(ImageGridView);
                if (Math.Abs(ptOverlay.Position.X - _lastClickPos.X) < 10 && Math.Abs(ptOverlay.Position.Y - _lastClickPos.Y) < 10)
                {
                    CloseViewer_Click(null, null);
                    _isDoubleClickDrag = true;
                    this.ViewModel.IsSelectMode = true;
                    _dragSelectStartPos = ptOverlay.Position;
                    _doubleClickStartItem = this.ViewModel.SelectedItem;
                    _previousSelection = this.ViewModel.SelectedItems.ToList();
                    ImageGridView.CapturePointer(e.Pointer);
                    return;
                }
            }
            ...
            {
                var eSrc = e.OriginalSource as DependencyObject;

                bool clickedOnContent = false;
                DependencyObject current = eSrc;

                while (current != null && current != ViewerOverlay)
                {
                    if (current == ViewerImage || current == ViewerVideo || current == BottomActionPanel || current == TopTrafficLights)
                    {
        ...
        private void FolderListView_DragItemsCompleted(ListViewBase sender, DragItemsCompletedEventArgs args)
        {
            this.ViewModel.SaveFolders();
        }
        ...
        private async void AddFolder_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (folder != null)
            {
                var newFolder = new FolderItem { Name = folder.Name, Path = folder.Path };
                this.ViewModel.Folders.Add(newFolder);
                this.ViewModel.SaveFolders();
                this.ViewModel.SelectedFolder = newFolder;
            }
        }

        private async void OpenFolderDirectory_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as MenuFlyoutItem)?.DataContext is FolderItem folderItem)
            {
                try
                {
        ...
        private async void RenameFolder_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as MenuFlyoutItem)?.DataContext is FolderItem folderItem)
            {
                var tb = new TextBox { Text = folderItem.Name };
                var dialog1 = new ContentDialog { Title = "Rename Folder", Content = tb, PrimaryButtonText = "Next", CloseButtonText = "Cancel", XamlRoot = this.RootGrid.XamlRoot };

                if (await dialog1.ShowAsync() == ContentDialogResult.Primary && !string.IsNullOrWhiteSpace(tb.Text))
                {
                    string newName = tb.Text.Trim();
                    var dialog2 = new ContentDialog
                    {
                        Title = "Rename Scope",
                        Content = "Only rename it within this instance or proceed with renaming it in the system?",
                        PrimaryButtonText = "System",
                        SecondaryButtonText = "Instance Only",
        ...
        private async void CopyFolder_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as MenuFlyoutItem)?.DataContext is FolderItem folderItem)
            {
                try
                {
                    ...
                    dataPackage.SetStorageItems(new List<IStorageItem> { folder });
                    Clipboard.SetContent(dataPackage);
                    ShowCopiedFlyout(_lastPointerPosition);
                }
                catch { }
            }
        }
        ...
        private async void DeleteFolder_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as MenuFlyoutItem)?.DataContext is FolderItem folderItem)
            {
                var dialog = new ContentDialog { Title = "Delete Folder", Content = $"Are you sure you want to move '{folderItem.Name}' to the Recycle Bin?", PrimaryButtonText = "Delete", CloseButtonText = "Cancel", XamlRoot = this.RootGrid.XamlRoot };
                if (await dialog.ShowAsync() == ContentDialogResult.Primary)
                {
                    try
                    {
                        ...
                        this.ViewModel.RemoveFolder(folderItem);
                    }
                    catch { }
                }
            }
        ...
        private void ShowCopiedFlyout(Point location)
        {
            var f = new Flyout();
            f.Content = new TextBlock { Text = "Copied to clipboard!", Margin = new Thickness(10) };
            f.ShowAt(this.RootGrid, new FlyoutShowOptions { Position = location, Placement = FlyoutPlacementMode.Top });
        }

        private async void SingleItemLocation_Click(object sender, RoutedEventArgs e)
        {
            if (_rightClickedItem == null) return;
            try
            {
                StorageFile f = await StorageFile.GetFileFromPathAsync(_rightClickedItem.FilePath);
                await Launcher.LaunchFolderAsync(await f.GetParentAsync(), new FolderLauncherOptions { ItemsToSelect = { f } });
            }
            catch { }
        }

        private async void SingleItemRename_Click(object sender, RoutedEventArgs e)
        {
            if (_rightClickedItem == null) return;
            var tb = new TextBox { Text = _rightClickedItem.Title };
            var dialog = new ContentDialog { Title = "Rename", Content = tb, PrimaryButtonText = "Rename", CloseButtonText = "Cancel", XamlRoot = this.RootGrid.XamlRoot };
            if (await dialog.ShowAsync() == ContentDialogResult.Primary)
            {
                try
                {
                    StorageFile file = await StorageFile.GetFileFromPathAsync(_rightClickedItem.FilePath);
                    string ext = file.FileType;
                    string newName = tb.Text.EndsWith(ext, StringComparison.OrdinalIgnoreCase) ? tb.Text : tb.Text + ext;
                    await file.RenameAsync(newName);
                    _rightClickedItem.Title = newName;
                    _rightClickedItem.FilePath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(_rightClickedItem.FilePath) ?? string.Empty, newName);
                }
                catch { }
            }
        }

        private void SingleItemCopy_Click(object sender, RoutedEventArgs e)
        {
            if (_rightClickedItem == null) return;
            var dataPackage = new DataPackage { RequestedOperation = DataPackageOperation.Copy };
            _ = Task.Run(async () => {
        ...
        private async void SingleItemDelete_Click(object sender, RoutedEventArgs e)
        {
            if (_rightClickedItem == null) return;
            MediaItem? temp = this.ViewModel.SelectedItem;
            this.ViewModel.SelectedItem = _rightClickedItem;
            await this.ViewModel.DeleteCurrentItem();
            this.ViewModel.SelectedItem = temp;
        }

        // pre: user invoked bulk rename on selected items
        // post: triggers view model rename action if valid name provided
        // except: none
        // complexity: O(1) for ui interaction
        private async void BulkRename_Click(object sender, RoutedEventArgs e)
        {
            var nameBox = new TextBox { PlaceholderText = "Enter base filename...", Width = 300 };
            var dialog = new ContentDialog
            {
                Title = "Bulk Rename",
                Content = nameBox,
            ...
            {
                await this.ViewModel.BulkRenameItems(nameBox.Text.Trim());
            }
        }

        private void EditTags_Click(object sender, RoutedEventArgs e)
        {
            var itemsToEdit = this.ViewModel.IsSelectMode ? this.ViewModel.SelectedItems.ToList() : new List<MediaItem>();
            if (!this.ViewModel.IsSelectMode && _rightClickedItem != null)
                itemsToEdit.Add(_rightClickedItem);

            if (itemsToEdit.Count == 0) return;

            foreach (var tag in this.ViewModel.Tags)
            {
                if (!tag.IsEditable && tag.Name != "Favorites") continue;
                bool allHaveTag = false;
                if (tag.Name == "Favorites")
                    allHaveTag = itemsToEdit.All(i => i.IsFavorite);
                else
                    allHaveTag = itemsToEdit.All(i => i.Tags.Contains(tag.Name));
                tag.IsCheckedForEdit = allHaveTag;
            }

            if (this.RootGrid.Resources["EditTagsFlyout"] as Flyout is Flyout flyout)
            {
                if (flyout.Content is FrameworkElement fe) fe.DataContext = this.ViewModel.Tags;
                var pt = _lastPointerPosition;
                this.DispatcherQueue.TryEnqueue(() =>
                {
                    flyout.ShowAt(ImageGridView, new FlyoutShowOptions { Position = pt, Placement = FlyoutPlacementMode.Right });
                });
            }
        }

        private void EditTagsCancel_Click(object sender, RoutedEventArgs e)
        {
            if (this.RootGrid.Resources["EditTagsFlyout"] as Flyout is Flyout flyout) flyout.Hide();
        }

        private void EditTagsDone_Click(object sender, RoutedEventArgs e)
        {
            var itemsToEdit = this.ViewModel.IsSelectMode ? this.ViewModel.SelectedItems.ToList() : new List<MediaItem>();
            if (!this.ViewModel.IsSelectMode && _rightClickedItem != null)
                itemsToEdit.Add(_rightClickedItem);
            ...
            {
                var tagsToAdd = this.ViewModel.Tags.Where(t => (t.IsEditable || t.Name == "Favorites") && t.IsCheckedForEdit == true).Select(t => t.Name).ToList();
                var tagsToRemove = this.ViewModel.Tags.Where(t => (t.IsEditable || t.Name == "Favorites") && t.IsCheckedForEdit == false).Select(t => t.Name).ToList();

                this.ViewModel.BulkEditTags(itemsToEdit, tagsToAdd, tagsToRemove);
            }

            if (this.RootGrid.Resources["EditTagsFlyout"] as Flyout is Flyout flyout) flyout.Hide();
            if (this.ViewModel.IsSelectMode) this.ViewModel.ClearSelection();
        }

        private void EditFlyoutNewTag_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button btn && btn.Parent is Grid grid && grid.Children[0] is TextBox textBox)
            {
                string newTag = textBox.Text.Trim();
                if (!string.IsNullOrEmpty(newTag))
                {
                    this.ViewModel.AddNewTag(newTag);
                    var tagItem = this.ViewModel.Tags.FirstOrDefault(t => t.Name == newTag);
                    if (tagItem != null) tagItem.IsCheckedForEdit = true;
                    textBox.Text = "";
                }
            }
        }

        private async void SingleItemRestore_Click(object sender, RoutedEventArgs e)
        {
            if (_rightClickedItem != null)
            {
                await this.ViewModel.RestoreItem(_rightClickedItem);
            }
        }

        private async void BulkRestore_Click(object sender, RoutedEventArgs e)
        {
            await this.ViewModel.BulkRestoreSelected();
        }

        private void BulkCopy_Click(object sender, RoutedEventArgs e)
        {
            this.ViewModel.CopySelectedToClipboard();
            ShowCopiedFlyout(_lastPointerPosition);
            this.ViewModel.ClearSelection();
        }
        ...
        private void FocusItem_Click(object sender, RoutedEventArgs e)
        {
            if (sender is FrameworkElement fe && fe.DataContext is MediaItem item) _ = OpenMedia(item, force: true);
        }
        ...
        private void SelectBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MenuFlyout? flyout = this.RootGrid.Resources["SelectionMenu"] as MenuFlyout;
                if (sender is FrameworkElement fe && flyout != null) flyout.ShowAt(fe);
            }
        }

        private MediaItem? GetMediaItemFromElement(object originalSource)
        {
            DependencyObject? depObj = originalSource as DependencyObject;
            while (depObj != null && depObj != ImageGridView) { if (depObj is FrameworkElement fe && fe.DataContext is MediaItem item) return item; depObj = VisualTreeHelper.GetParent(depObj); }
            return null;
        }
        ...
        private void ImageGridView_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (isLeft || isRight)
            {
                ...
                {
                    long currentTicks = DateTime.Now.Ticks;
                    bool isDoubleClick = isLeft && (currentTicks - _lastClickTicks < TimeSpan.TicksPerMillisecond * 500) && 
                        (Math.Abs(pt.Position.X - _lastClickPos.X) < 10 && Math.Abs(pt.Position.Y - _lastClickPos.Y) < 10);
                    ...
                    if (this.ViewModel.IsSelectMode && shiftPressed && _lastSelectedAnchor != null)
                    {
                        this.ViewModel.SelectRange(_lastSelectedAnchor, clickedItem);
                        e.Handled = true;
                    }
                    else if (isDoubleClick)
                    {
                        ...
                        _doubleClickStartItem = clickedItem;
                        _previousSelection = this.ViewModel.SelectedItems.ToList();
                        ImageGridView.CapturePointer(e.Pointer);
                    }
                    else
                    {
                        _isDragSelecting = false;
                        _isDoubleClickDrag = false;
                        _isRightDrag = isRight;
                        _dragSelectStartPos = pt.Position;
                        _pressedItem = clickedItem;
                        ImageGridView.CapturePointer(e.Pointer);
                    }
                }
            }
        ...
        private void ImageGridView_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            if (ImageGridView.PointerCaptures != null && ImageGridView.PointerCaptures.Count > 0)
            {
                ...
                if (_doubleClickStartItem != null && !_isDoubleClickDrag && (Math.Abs(pt.Position.X - _lastClickPos.X) > 10 || Math.Abs(pt.Position.Y - _lastClickPos.Y) > 10))
                {
                    _isDoubleClickDrag = true;
                }

                if (!_isDragSelecting && !_isDoubleClickDrag && (Math.Abs(pt.Position.X - _dragSelectStartPos.X) > 10 || Math.Abs(pt.Position.Y - _dragSelectStartPos.Y) > 10))
                {
                        ...
                        if (element is FrameworkElement fe && fe.DataContext is MediaItem item)
                        {
                            ...
                            if (idx1 != -1 && idx2 != -1)
                            {
                                ...
                                for (int i = low; i <= high; i++)
                                    newRange.Add(this.ViewModel.MediaItems[i]);
                            }

                            var currentSelected = this.ViewModel.SelectedItems.ToList();
                            foreach(var sel in currentSelected)
                            {
                                if (!newRange.Contains(sel) && !_previousSelection.Contains(sel))
                                {
                                    this.ViewModel.ToggleItemSelection(sel);
                                }
                            }

                            foreach(var rItem in newRange)
                            {
                                if (!rItem.IsSelected)
                                {
                                    this.ViewModel.ToggleItemSelection(rItem);
                                }
                            }
                            
                            foreach(var prev in _previousSelection)
                            {
                                if (!prev.IsSelected)
                                {
                                    this.ViewModel.ToggleItemSelection(prev);
                                }
                            }
        ...
        {
            ImageGridView.ReleasePointerCapture(e.Pointer);
            MediaItem? releasedItem = GetMediaItemFromElement(e.OriginalSource);
            var pt = e.GetCurrentPoint(ImageGridView);
            _lastPointerPosition = pt.Position;

            if (_isDoubleClickDrag)
            {
                _isDoubleClickDrag = false;
                _doubleClickStartItem = null;
            }
            else if (_isDragSelecting)
            {
                _isDragSelecting = false;
                if (_isRightDrag)
                {
                    ...
                    if (flyout != null)
                    {
                        var restoreItem = flyout.Items.OfType<MenuFlyoutItem>().FirstOrDefault(i => i.Name == "BulkRestoreMenu");
                        if (restoreItem != null) restoreItem.Visibility = this.ViewModel.SelectedItems.Any(x => x.IsDeleted) ? Visibility.Visible : Visibility.Collapsed;
                        flyout.ShowAt(ImageGridView, pt.Position);
                    }
                }
            }
            else
            {
                if (releasedItem != null && (releasedItem == _pressedItem || releasedItem == _doubleClickStartItem))
                {
                    if (_isRightDrag)
                    {
                        if (this.ViewModel.IsSelectMode && releasedItem.IsSelected)
                        {
                            MenuFlyout? flyout = this.RootGrid.Resources["SelectionMenu"] as MenuFlyout;
                            if (flyout != null)
                            {
                        ...
                        {
                            _rightClickedItem = releasedItem;
                            MenuFlyout? flyout = this.RootGrid.Resources["SingleItemMenu"] as MenuFlyout;
                            if (flyout != null)
                            {
                                var restoreItem = flyout.Items.OfType<MenuFlyoutItem>().FirstOrDefault(i => i.Name == "SingleItemRestoreMenu");
                                if (restoreItem != null) restoreItem.Visibility = releasedItem.IsDeleted ? Visibility.Visible : Visibility.Collapsed;
                                flyout.ShowAt(ImageGridView, pt.Position);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            this.ViewModel.ToggleItemSelection(releasedItem);
                            _lastSelectedAnchor = releasedItem;
                        }
                        else
                        {
                            _ = OpenMedia(releasedItem);
                        }
                    }
                }
            }
        ...
        // pre: valid media item is selected and loaded
        // post: window dimensions are animated to match the aspect ratio of the media
        // except: none
        // complexity: O(1) mostly
        private void ScaleWindow_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (aspectRatio > 0)
            {
                double currentContentHeight = this.RootGrid.ActualHeight;
                double currentContentWidth = this.RootGrid.ActualWidth;
                double targetContentWidth = currentContentHeight * aspectRatio;
                double widthDelta = targetContentWidth - currentContentWidth;
                int currentWindowWidth = this.AppWindow.Size.Width;
                int currentWindowHeight = this.AppWindow.Size.Height;
                int targetWindowWidth = (int)Math.Round(currentWindowWidth + widthDelta);
                int targetWindowHeight = currentWindowHeight;

                if (targetWindowWidth < 400)
                {
                    targetWindowWidth = 400;
                    double clampedContentWidth = currentContentWidth + (targetWindowWidth - currentWindowWidth);
                    double targetContentHeight = clampedContentWidth / aspectRatio;
                    targetWindowHeight = (int)Math.Round(currentWindowHeight + (targetContentHeight - currentContentHeight));
                }
        ...
        // pre: valid MediaItem is provided, viewer is not locked
        // post: viewer overlay is visible, media is loaded and displayed
        // except: none
        // complexity: O(1) mostly, or O(loading thumbnail/video)
        private async Task OpenMedia(MediaItem item, bool force = false)
        {
            ...
            this.ViewerOverlay.Visibility = Visibility.Visible;
            this.MediaScrollViewer.ChangeView(0, 0, 1.0f, true);

            UpdateActionPanelLayout();
            AnimateHud(true);

            if (item.IsVideo) { this.ViewerImage.Visibility = Visibility.Collapsed; this.ViewerVideo.Visibility = Visibility.Visible; try { StorageFile file = await StorageFile.GetFileFromPathAsync(item.FilePath); this.ViewerVideo.Source = MediaSource.CreateFromStorageFile(file); } catch { } }
            else { this.ViewerVideo.Visibility = Visibility.Collapsed; this.ViewerImage.Visibility = Visibility.Visible; this.ViewerVideo.Source = null!; try { StorageFile file = await StorageFile.GetFileFromPathAsync(item.FilePath); using var stream = await file.OpenAsync(FileAccessMode.Read); BitmapImage bmp = new BitmapImage(); await bmp.SetSourceAsync(stream); this.ViewerImage.Source = bmp; } catch { this.ViewerImage.Source = item.Thumbnail; } }
        ...
        {
            var t = new TextBox { PlaceholderText = "Tag name", Width = 300 };
            var autoTagCheck = new CheckBox { Content = "Auto-tag files sharing this name", Margin = new Thickness(0, 8, 0, 0) };

            var stack = new StackPanel();
            stack.Children.Add(t);
            stack.Children.Add(autoTagCheck);

            if (await new ContentDialog { Title = "New Tag", Content = stack, PrimaryButtonText = "Create", CloseButtonText = "Cancel", XamlRoot = this.RootGrid.XamlRoot }.ShowAsync() == ContentDialogResult.Primary)
            {
                string tagName = t.Text.Trim();
                this.ViewModel.AddNewTag(tagName);

                if (autoTagCheck.IsChecked == true)
                {
                    await this.ViewModel.AutoTagByFilename(tagName);
                }
            }
        }
        ...
        private void TagBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (var tag in this.ViewModel.Tags)
            {
                if (tag.Name == "All Photos") continue;
                var i = new ToggleMenuFlyoutItem { Text = tag.Name, IsChecked = this.ViewModel.SelectedItem?.Tags.Contains(tag.Name) ?? false };
                i.Click += (s, a) => this.ViewModel.ToggleTagForSelection(tag.Name);
                f.Items.Add(i);
            }
            if (sender is Button btn) { btn.Flyout = f; f.ShowAt(btn); }
            else if (sender is FrameworkElement fe) { f.ShowAt(fe); }
        }
